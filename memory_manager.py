...
class AdaptiveThrottler:
    ...
    def step(self) -> None:
        """Apply backpressure if queue depth exceeds target fraction."""
...
def _pin_thread_to_numa_node(node: int) -> None:
    """Pin calling thread to CPUs of the given NUMA node."""
...
class SpillManager:
    ...
    def spill_async(self, key: str, generation: int, tensor: torch.Tensor, old_generation: int = -1) -> None:
        """Enqueue a spill write; fall back to sync if queue full."""
        for _ in range(5):
            try:
...
class EvictionPolicy:
    ...
    Heap-backed policies are O(log n) for victim selection.
    """
...
class MemoryManager:
    ...
    def defragment_gpu(self, device: Optional[torch.device] = None, pressure_threshold: float = 0.70) -> int:
        """
        Compact GPU allocations by copying each live tensor to a fresh
        contiguous buffer, then releasing the original. Returns bytes moved.
        """
        if not torch.cuda.is_available():
            return 0
